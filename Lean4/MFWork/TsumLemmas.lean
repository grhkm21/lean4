...
@[norm_cast]
theorem tendsto_coe {f : Filter α} {m : α → ℝ} {a : ℝ} :
    Tendsto (fun a => (m a : ℂ)) f (𝓝 ↑a) ↔ Tendsto m f (𝓝 a) :=
  embedding_coer.tendsto_nhds_iff.symm

@[simp, norm_cast]
theorem coe_finset_sum' {s : Finset α} {f : α → ℝ} :
    ↑(∑ a in s, f a) = (∑ a in s, f a : ℂ) :=
  ofReal_sum s f

@[norm_cast]
theorem hasSum_coe {f : α → ℝ} {r : ℝ} :
...
theorem tsum_coe_eq {f : α → ℝ} {r : ℝ} (h : HasSum f r) :
    ∑' a, (f a : ℂ) = r :=
  (hasSum_coe.2 h).tsum_eq

theorem coe_tsum {f : α → ℝ} : Summable f → ↑(tsum f) = ∑' a, (f a : ℂ)
  | ⟨r, hr⟩ => by rw [hr.tsum_eq, tsum_coe_eq hr]

theorem coe_summable (f : α → ℝ) :
...
theorem tsum_coe (f : α → ℝ) : ∑' i, (f i : ℂ) = (∑' i, f i : ℝ) :=
  by
...
theorem nat_pos_tsum2 [TopologicalSpace α] [AddCommMonoid α]
...
-- TODO
axiom nat_pos_tsum2' [TopologicalSpace α] [AddCommGroup α] (f : ℕ → α) :
    (Summable fun x : ℕ+ => f x) ↔ Summable f

theorem hasSum_pnat' (f : ℕ → ℂ) (hf2 : Summable f) :
    HasSum (fun n : ℕ => f (n + 1)) (∑' n : ℕ+, f n) := by
  have hf3 : Summable ((fun n : ℕ => f (n + 1)) ∘ PNat.natPred) :=
    ...
    intro b; simp
  rw [← _root_.Equiv.pnatEquivNat.hasSum_iff, Equiv.pnatEquivNat_apply, Summable.hasSum_iff hf3]
  congr
  funext
  simp
...
theorem tsum_pNat [AddCommGroup α] [UniformSpace α] [UniformAddGroup α] [T2Space α]
    [CompleteSpace α] (f : ℕ → α) (hf : Summable f) :
      HasSum (fun n : ℕ => f (n + 1)) (∑' n : ℕ+, f n) := by
  rw [← _root_.Equiv.pnatEquivNat.hasSum_iff]
  simp_rw [Equiv.pnatEquivNat] at *
  simp at *
  have hf3 : Summable ((fun n : ℕ => f (n + 1)) ∘ PNat.natPred) := by
    apply Summable.congr (hf.subtype _)
    intro b
    simpa using by rfl
  simp [Summable.hasSum_iff hf3]

theorem tsum_pNat' [AddCommGroup α] [UniformSpace α] [UniformAddGroup α] [T2Space α]
    [CompleteSpace α] (f : ℕ → α) : ∑' n : ℕ+, f n = ∑' n, f (n + 1) := by
  by_cases hf2 : Summable f
  · rw [eq_comm, (tsum_pNat f hf2).tsum_eq]
  · rw [tsum_eq_zero_of_not_summable, tsum_eq_zero_of_not_summable]
    · rwa [← nat_pos_tsum2'', nat_pos_tsum2']
    · rwa [nat_pos_tsum2']

theorem tsum_pNat'' [AddCommGroup α] [UniformSpace α] [UniformAddGroup α] [T2Space α]
    [CompleteSpace α] (f : ℕ → α) (hf : f 0 = 0) : ∑' n : ℕ+, f n = ∑' n, f n := by
  by_cases hf2 : Summable f
  · rw [tsum_eq_zero_add hf2, hf, zero_add, eq_comm, (tsum_pNat _ hf2).tsum_eq]
...
theorem summable_mul_prod_iff_summable_mul_sigma_antidiagonall [TopologicalSpace α]
...
theorem hasDerivAt_tsum_fun (f : α → ℂ → ℂ)
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (K «expr ⊆ » s) -/
theorem hasDerivWithinAt_tsum_fun (f : α → ℂ → ℂ)
    {s : Set ℂ} (hs : IsOpen s) (x : ℂ) (hx : x ∈ s)
    (hf : ∀ y : ℂ, y ∈ s → Summable fun n : α => f n y)
    (hu :
      ∀ (K) (_ : K ⊆ s),
...
/- ./././Mathport/Syntax/Translate/Basic.lean:635:2: warning: expanding binder collection (K «expr ⊆ » s) -/
theorem deriv_tsum_fun' (f : α → ℂ → ℂ) {s : Set ℂ}
